...
- Verification: architecture, lint, exact TypeScript, build, Prisma schema,
  ...
  tests passed 7 tests. After remote compatibility-route reconciliation,
  controller and Postman inventories both contain 140
  unique method/path pairs with zero missing, obsolete, or duplicate routes.
- Database/client impact: the Postman/GitHub refactor itself introduced no
  migration or schema change, and no `client/` file was edited.
...
### 2026-08-10 - Remote reconciliation: semantic skill-generation notifications

- Scope: reconciled the incoming GitHub evidence and skill-profile generation
  work with the newer durable Notification architecture. Contributor and active
  admin generation outcomes now use versioned semantic templates, atomic
  `NotificationEvent` creation, bounded private parameters, bilingual
  presentation, recipient-specific deduplication, and shared `/realtime`
  publication instead of restoring the retired process-local gateway.
- HTTP compatibility: retained the cursor/filter/read-state/preferences API and
  kept the incoming `PATCH /notifications/:notificationId/read` and
  `PATCH /notifications/read-all` command paths as adapters to the same owning
  inbox service. `GET /notifications` now includes the unread count additively.
- Migration ordering: the three incoming skill-generation enum/backfill
  migrations run before the unpublished semantic migration, which now maps
  valid generation outcomes to their semantic templates and safely preserves
  any malformed legacy row through the generic fallback.
- Verification: focused merge regressions passed 46 tests; the isolated
  semantic migration harness passed; full Jest passed 120 suites / 792 tests

...
import { SkillProfileGenerationProcessorService } from './skill-profile-generation-processor.service';
...
import { SkillProfileGenerator } from '../../../ai/application/ports/skill-profile-generator.port';
...
function createProcessor() {
  ...
  return {
    processor: new SkillProfileGenerationProcessorService(
      generations as unknown as SkillProfileGenerationRepository,
      gitHubRepositoryService as unknown as GitHubRepositoryService,
      skillProfileGenerator as unknown as SkillProfileGenerator,
...
describe('SkillProfileGenerationProcessorService', () => {
  it('collects evidence, calls AI, and stores high-confidence pending skills', async () => {
    ...
    expect(skillProfileGenerator.generate).toHaveBeenCalledWith(
  ...
  it('throws when AI processing fails so BullMQ can retry it', async () => {
    const { processor, generations, skillProfileGenerator } = createProcessor();
    skillProfileGenerator.generate.mockRejectedValueOnce(new Error('AI down'));
  ...
  it('stores an insufficient-evidence result without pending skills', async () => {
    const { processor, generations, skillProfileGenerator } = createProcessor();
    skillProfileGenerator.generate.mockResolvedValueOnce({
      skills: [
        {
          name: 'TypeScript',
          proficiency: 'intermediate',

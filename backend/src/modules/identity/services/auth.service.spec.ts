...
function createService() {
  ...
  const sessionService = {
    ...
    canAuthenticate: jest
      .fn()
      .mockImplementation(
        (user: User) =>
          user.status === UserStatus.active ||
          (user.role === UserRole.contributor && user.status === UserStatus.pending),
      ),
  };
...
describe('AuthService', () => {
  ...
  it('verifies a valid OTP, activates the user, and creates a session', async () => {
    const { database, service, sessionService } = createService();
    ...
    expect(sessionService.create).toHaveBeenCalledWith(activeUser.id, {
      userAgent: 'jest',
      ipAddress: '127.0.0.1',
    });
  ...
  it('allows pending contributors to login', async () => {
    const { database, passwordHasher, service } = createService();
